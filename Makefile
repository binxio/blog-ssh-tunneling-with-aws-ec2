...
private-dns: ## shows the private ip address
	@sceptre --output json describe-stack-outputs example vpc | jq -r '.[] | select(.OutputKey=="PrivateHostDnsName") | .OutputValue'

tunnel: write-private-key ## create ssh tunnel
	ssh -N -L8080:$(shell sceptre --output json describe-stack-outputs example vpc | jq -r '.[] | select(.OutputKey=="PrivateHostDnsName") | .OutputValue'):80 -i bastion.pem ec2-user@$(shell sceptre --output json describe-stack-outputs example vpc | jq -r '.[] | select(.OutputKey=="BastionHostPublicDnsName") | .OutputValue')